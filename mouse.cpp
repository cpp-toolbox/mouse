 ...
 *
 * \note In radians tau ~= 6.28 represents a full rotation, and on average we can assume that a
 * a player usually wants to rotate their view angle by theta := tau/4 ~= 1.57, also note that
 * delta mouse positions are coming in at (probably) a rate of at least 60Hz, which means that
 * if a player wants to rotate their view by theta in one second they must at least be
 * producing deltas of size theta/60 = 0.0261.
 *
 * On average it seems that most mice produce deltas of size TODO, which are much greater than
 * this value, and a general rule of thumb is that to produce good delta angles, we should scale down
 * our delta by 1000x
 *
